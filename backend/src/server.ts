...
app.get('/api/admin/session', (req: AuthenticatedRequest, res) => {
  res.json({ success: true, data: req.user });
});

app.put('/api/admin/festival', async (req: AuthenticatedRequest, res) => {
  if (!db) {
...
app.put('/api/admin/finance', async (req: AuthenticatedRequest, res) => {
  if (!db) {
    res.status(503).json({ success: false, error: { code: 'FIREBASE_NOT_CONFIGURED', message: 'Firebase Admin is not configured.' } });
    return;
  }
...
app.post('/api/admin/announcements', async (req: AuthenticatedRequest, res) => {
  if (!db) {
    res.status(503).json({ success: false, error: { code: 'FIREBASE_NOT_CONFIGURED', message: 'Firebase Admin is not configured.' } });
    return;
  }
...
app.put('/api/admin/events/:eventId', async (req: AuthenticatedRequest, res) => {
  if (!db) {
    res.status(503).json({ success: false, error: { code: 'FIREBASE_NOT_CONFIGURED', message: 'Firebase Admin is not configured.' } });
    return;
  }
...
app.delete('/api/admin/events/:eventId', async (req: AuthenticatedRequest, res) => {
  if (!db) {
    res.status(503).json({ success: false, error: { code: 'FIREBASE_NOT_CONFIGURED', message: 'Firebase Admin is not configured.' } });
    return;
  }
...
app.listen(env.PORT, () => {
  ...
  if (!hasFirebaseAdminConfig) console.warn('Firebase Admin is not configured; protected data operations are disabled.');
});
